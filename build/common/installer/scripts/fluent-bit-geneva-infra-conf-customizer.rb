...
@fluent_bit_conf_path = "/etc/opt/microsoft/docker-cimprov/fluent-bit-geneva-logs_infra.conf"

@os_type = ENV["OS_TYPE"]
if !@os_type.nil? && !@os_type.empty? && @os_type.strip.casecmp("windows") == 0
  @fluent_bit_conf_path = "/etc/fluent-bit/fluent-bit-geneva-logs_infra.conf"
end
...
def substituteFluentBitPlaceHolders
  begin
    ...
    puts "config::Starting to substitute the placeholders in fluent-bit-geneva-logs_infra.conf file for log collection"
    ...
    ConfigParseErrorLogger.logError("fluent-bit-geneva-tenant-conf-customizer: error while substituting values in fluent-bit-geneva-logs_infra.conf file: #{errorStr}")
  end
end
