...
def substituteFluentBitPlaceHolders
  begin
    ...
    puts "config::Starting to substitute the placeholders in td-agent-bit-geneva-logs_tenant.conf file for log collection"
    ...
    ConfigParseErrorLogger.logError("td-agent-bit-geneva-tenant-conf-customizer: error while substituting values in td-agent-bit-geneva.conf file: #{errorStr}")
  end
end
