...
func (plug *perfPlugin) Init(ctx context.Context, fbit *plugin.Fluentbit) error {
	if fbit.Conf.String("tag") == "" {
		plug.tag = tag
	} else {
		plug.tag = fbit.Conf.String("tag")
	}
	if fbit.Conf.String("run_interval") == "" {
		plug.runInterval = runInterval
	} else {
		plug.runInterval, _ = strconv.Atoi(fbit.Conf.String("run_interval"))
	}
	if fbit.Conf.String("insightsmetricstag") == "" {
		plug.insightsmetricstag = insightsmetricstag
	} else {
		plug.insightsmetricstag = fbit.Conf.String("insightsmetricstag")
	}
...
func (plug perfPlugin) Collect(ctx context.Context, ch chan<- plugin.Message) error {
	tick := time.NewTicker(time.Duration(plug.runInterval) * time.Second)
	...
	for {
		...
		case <-tick.C:
			...
			perfmessages, insightsmetricsmessages := plug.enumerate()
			...
			if timeDifferenceInMinutes >= 5 {
				...
				lib.SendTelemetry("PluginName", telemetryProperties) // Replace "PluginName" with the actual plugin name
			}
		}
	}
}

func (plug perfPlugin) enumerate() ([]map[string]interface{}, []map[string]interface{}) {
	...
	tag = plug.tag
	insightsmetricstag = plug.insightsmetricstag
