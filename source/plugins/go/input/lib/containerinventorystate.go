...
var inventoryDirectory = "/var/opt/microsoft/docker-cimprov/state/ContainerInventory/"
...
func WriteContainerState(container Container) {
	...
	if containerId != "" {
		file, err := os.Create(filepath.Join(inventoryDirectory, containerId))
		if err != nil {
...
func ReadContainerState(containerId string) (containerObject Container) {
	filepath := filepath.Join(inventoryDirectory, containerId)
...
func GetDeletedContainers(containerIds []string) (deletedContainers []string) {
	files, err := ioutil.ReadDir(inventoryDirectory)
	if err != nil {
