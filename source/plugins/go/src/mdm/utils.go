...
func GetAllContainerLimits() (map[string]float64, map[string]float64, map[string]string, error) {
	...
	for _, item := range podInventory["items"].([]interface{}) {
		...
		Log.Printf("MDMLog: podName, podNamespace, podUid: %s, %s, %s", podName, podNamespace, podUid)
		if podUid == "" {
			continue
		}
	...
	Log.Printf("MDMLog: containerCpuLimitHash: %v", containerCpuLimitHash)
	Log.Printf("MDMLog: containerMemoryLimitHash: %v", containerMemoryLimitHash)
	Log.Printf("MDMLog: containerResourceDimensionHash: %v", containerResourceDimensionHash)
