...
func getDataTypeToStreamIdMapping() (map[string]string, error) {
	...
	for _, extensionConfig := range extensionConfigs {
		...
		isContainerLogV2, isContainerLogV2Exists := dataCollectionSettings["enableContainerLogV2"]
		if isContainerLogV2Exists && isContainerLogV2.(bool) {
			logger.Printf("isContainerLogV2 value in DCR settings: %s", isContainerLogV2)
			datatypeOutputStreamMap["isContainerLogV2"] = "true"
		} else {
			logger.Printf("containerLogV2 is not set in DCR settings")
		}
	}
	logger.Printf("Info::mdsd::build the datatype and streamid map -- end")
